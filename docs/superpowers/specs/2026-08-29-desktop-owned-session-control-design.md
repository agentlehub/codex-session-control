...
Drive the built CSC binary over stdio MCP, assert the exact catalog, then exercise all thirteen tools on the ledger-owned tasks. On normal error or panic, unconditionally stop/reap the MCP child, recover only IDs belonging to the exact unique workspace, persist them, archive ledger IDs through a fresh native connection, and verify each is absent from active listings and present in archived listings. Delete the run directory only after cleanup proof. On cleanup failure, retain and report the exact ledger path and remaining IDs.
